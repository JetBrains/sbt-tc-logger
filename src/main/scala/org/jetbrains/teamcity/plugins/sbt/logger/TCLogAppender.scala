...
class TCLogAppender extends LogAppender {
  ...
  private[logger] def compilationBlockStartCallback(flowId: String, projectName: Option[String]): () => Unit =
    guardedCompilationStart(flowId, projectName, inTest = false)

  private[logger] def compilationTestBlockStartCallback(flowId: String, projectName: Option[String]): () => Unit =
